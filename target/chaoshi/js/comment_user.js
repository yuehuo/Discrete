layui.use(['laydate', 'laypage', 'layer', 'table', 'form', 'upload'], function() {
    ...
    table.render({
        elem: '#commentuser'
        ,height: 500
        ,url: '/discrete/admin/comment/usercommentlist' //后端的评论列表接口
        ,title: '评论列表'
        ...
        ,cols: [[ //表头
            ...
            ,{field: 'uProfile', title: '发布者头像',templet:function (d) {
                    //d:是每行对应的用户对象
    ...
    table.on('toolbar(commentuser)', function(obj){
    ...
    form.on('submit(comment_add)', function(data){
        ...
        $.ajax({
            url:"/discrete/admin/comment/add",//后端添加歌手的接口
            type:"post",
    ...
    // 监听行工具栏事件：删除评论与更新评论
    table.on('tool(commentuser)', function(obj) {
        // 获取当前行数据和lay-event的值
        var data = obj.data;
        var event = obj.event;
