layui.use(['laydate', 'laypage', 'layer', 'table', 'form', 'upload'], function() {
    ...
    table.render({
        elem: '#documentuser'
        ,height: 500
        ,url: '/discrete/admin/document/userdocumentlist' //后端的文档列表接口
        ,title: '文档列表'
        ...
        ,cols: [[ //表头
            ...
            ,{field: 'name', title: '文档名字'}
            ,{field: 'path', title: '文档地址',templet:function (d) {
    ...
    var uploadInst = upload.render({
        elem: '#chooseDocument'//上传文件按钮的id
        , url: '/discrete/admin/document/upload' //上传音频接口
        ,data:{  //上传时提交的参数
            parentName:"document",//存储文件的父文件夹
        }
        ,field:"uploadFile"//指定文件域的字段名，跟后端的MultipartFile保持一致
        ,accept:"file"//接受任何文件上传
        ,auto:true
        // , before: function (obj) {
        //     //上传之后回显
    ...
    table.on('toolbar(documentuser)', function(obj){
        ...
        switch(obj.event){
            ...
            case 'deleteBatch':
                ...
                if(data.length === 0){
                    ...
                    $.ajax({
                        url:"/discrete/admin/document/deleteBatch",
                        type:"post",
                        data:{
                            ids:ids
                        },
    ...
    // 监听行工具栏事件：删除文档与更新文档
    table.on('tool(documentuser)', function(obj) {
        // 获取当前行数据和lay-event的值
        var data = obj.data;
        var event = obj.event;
        ...
        if (event === 'deleteDocument') {
            layer.confirm('确定删除该文件吗？', function(index) {
                // ajax方式删除文档
                $.ajax({
                    url:"/discrete/admin/document/deleteDocument",
                    type:"post",
                    data:{
                        id:data.id
                    },
        ...
        // 更新文档事件
        if (event === 'updateDocument') {
            // 显示更新文档表单的弹出层
            console.log(data)
            var id = data.id;
            console.log("/discrete/update_document?id="+id)
            layer.open({
                type: 2,
                title:"编辑文档",
                content:"/discrete/update_document?id="+id,
                area: ['80%', '70%'],//指定宽高
                shade:0.8,//调节遮罩层的透明度
                shadeClose:true
            });
    ...
    // 更新文档表单提交
    form.on('submit(document_update)', function(data) {
        // ajax方式更新订单
        console.log(data.field) //全部表单字段，名值对形式：{name: value}
        $.ajax({
            url:"/discrete/admin/document/updateDocument",
            type:"post",
            data: data.field,//data:向后端传递的参数
            success: function(res) {
                //父窗口重新加载
